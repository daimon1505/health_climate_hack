  ...
  Router.add("/methods", (app) => {
    ...
    app.innerHTML = `
      <section class="page narrow prose">
        ...
        <h2>2. Personal band (per patient)</h2>
        <p>Start from the neighborhood band, then go <strong>up one level</strong> for each rule that applies (never down, at most +${E.max_steps}):</p>
        <ol>
          <li><strong>Medication rule:</strong> takes a <em>high</em> heat-risk medicine and heat points ≥ ${E.medication_rule.min_env_points}.</li>
        ...
        <p>Yellow: the patient decides whether to report. Red: sent to the care team automatically, <em>only</em> if the patient consented.</p>
        ...
        <table><thead><tr><th>Class</th><th>Heat risk</th><th>Reviewed by</th></tr></thead><tbody>
          ${rules.map((r) => `<tr><td>${esc(r.label)}</td><td>${esc(r.risk_level)}${r.placeholder ? " (placeholder)" : ""}</td><td>${esc(r.reviewed_by)}</td></tr>`).join("")}
        </tbody></table>
        <p class="small">Source of truth: CDC “Heat and Medications — Guidance for Clinicians”.</p>
        ...
        <ul>
          ...
          <li>Historical temperatures: NOAA NCEI Daily Summaries, Central Park <em>(not yet loaded — placeholder)</em>.</li>
        </ul>
