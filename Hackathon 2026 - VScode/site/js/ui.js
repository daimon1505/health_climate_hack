...
window.UI = (function () {
  ...
  function bandPill(band, extraClass = "") {
    if (!band) return `<span class="pill band-none">– No data</span>`;
    return `<span class="pill band-${band.id} ${extraClass}"><span aria-hidden="true">${band.icon}</span> ${band.label}</span>`;
  }
  ...
  function unreviewedBanner(rules) {
    if (!rules.some((r) => r.reviewed_by === "UNREVIEWED")) return "";
    return banner("warn", "<strong>Unreviewed medication rules.</strong> These heat rules have not been checked against the CDC guidance by a clinician yet.");
  }

  function legend() {
    return `<div class="legend" aria-label="Map legend">
      ${Scoring.CONFIG.bands.map((b) => `<span><i class="swatch" style="background:${NycMap.FILL[b.id]}"></i>${b.icon} ${b.label}</span>`).join("")}
  ...
  function renderTopbar() {
    ...
    const nav = acct
      ? acct.type === "care"
        ? `<a href="#/care">Inbox</a>`
        : `<a href="#/patient/${acct.id}">My status</a><a href="#/patient/${acct.id}/meds">My medicines</a>`
      : "";
    document.getElementById("topbar").innerHTML = `
      <div class="topbar-inner">
        <a class="logo" href="#/"><span aria-hidden="true">🌡️</span> HeatSafe NYC</a>
        ...
        <div class="topbar-right">
          ...
          <button class="button button-ghost button-small" id="drawer-toggle" aria-expanded="false" aria-controls="drawer" aria-label="Demo controls">⚙<span class="drawer-label"> Demo controls</span></button>
        </div>
      </div>`;
  ...
  function renderDrawer() {
    ...
    drawer.innerHTML = `
      <div class="drawer-head"><h2>Demo controls</h2><button class="icon-button" id="drawer-close" aria-label="Close demo controls">✕</button></div>
